...
# Lembre-se que o arquivo precisa estar no seu diretório de trabalho ou forneça o caminho completo.
...
# Variáveis resposta
# ------------------------------------------------------------------
variaveis <- c("Produtividade", "MrP", "COS", "STAND")
...
for (var in variaveis) {
  ...
  tab <- as.data.frame(cld_res) %>%
    dplyr::rename( # <-- CORREÇÃO APLICADA AQUI
      Media = emmean,
      Erro = SE,
      Grupo = .group
    ) %>%
  ...
  # Descomente a linha abaixo para salvar os arquivos
