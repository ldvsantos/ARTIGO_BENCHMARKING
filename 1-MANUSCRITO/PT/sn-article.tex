...
O conceito de IQS consolidou-se a partir do arcabouço proposto por \cite{Karlen1997} e do protocolo operacional de \cite{Andrews2002}, que estabeleceu a seleção de conjunto mínimo de dados (MDS) por análise de componentes principais (ACP) seguida de escorificação aditiva. Desde então, revisões abrangentes documentaram a proliferação de variantes metodológicas, com mais de 60 combinações de seleção, ponderação e agregação publicadas até 2018 \citep{Bunemann2018}. Essa diversidade, embora sinal de maturidade da área, introduz um problema de comparabilidade entre estudos que adotam paradigmas distintos para os mesmos indicadores.
...
A aplicação deste mesmo \textit{framework} a bases provenientes de outras classes de solo, particularmente Latossolos argilosos do Cerrado, onde \cite{DosSantos2021} indicaram a eficácia do IQS fuzzy na diferenciação entre sistemas de manejo, constitui direção promissora para consolidar os limiares de concordância aqui propostos.
...
Os cinco paradigmas convergiram nos extremos do ranqueamento, confirmando parcialmente a hipótese de estruturas de variância compatíveis, porém essa convergência mostrou-se marcadamente heterogênea entre famílias analíticas, com os métodos não lineares (SIF, MARS, RF) formando um bloco praticamente intercambiável enquanto a ANOVA/MDS divergiu a ponto de a maioria das combinações intermediárias de manejo alterar de posição conforme o paradigma adotado. A expansão do vetor de indicadores para incluir domínios microbiológico, químico e de rendimento intensificou, em vez de atenuar, essa divergência interfamílias, sugerindo que avaliações multidimensionais de IQS são mais sensíveis à escolha do paradigma analítico do que avaliações restritas a um único domínio. A estabilização log-linear da concordância com o aumento do tamanho amostral, esperada pela terceira hipótese, não foi confirmada para o vetor de 34 indicadores, indicando que exercícios de benchmarking em alta dimensionalidade operam sob incerteza algorítmica persistente que a mera expansão amostral pode não resolver. O cultivo mínimo com leguminosas, particularmente guandu, parece constituir a recomendação de manejo mais consistente entre todas as abordagens testadas, ao passo que a mesma espécie sob preparo convencional ocupou a última posição, evidenciando a condicionalidade dos benefícios da cobertura vegetal à preservação da integridade física do solo mediada pelo tipo de preparo. Avaliações de IQS destinadas a subsidiar políticas de conservação podem beneficiar-se da inclusão de pelo menos um paradigma não linear e de análise de sensibilidade inter-paradigma explícita para posições intermediárias do ranqueamento.
...
O conjunto de dados anonimizado e todas as rotinas de análise (R e Python) utilizadas neste estudo encontram-se disponíveis em \url{https://doi.org/10.5281/zenodo.19390736}. As Tabelas Suplementares S1--S4 apresentam a saída completa da ANOVA, a base de regras fuzzy, os coeficientes de caminho do PLS-SEM e os ranqueamentos de importância de variáveis.
